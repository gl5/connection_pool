...
class TestConnectionPool < Minitest::Test
  ...
  def use_pool(pool, size)
    Array.new(size) { Thread.new { pool.with { sleep } } }.each do |thread|
  ...
  def test_nested_checkout
    ...
    assert_equal %w(inner outer other), recorder.calls
  end
  ...
  def test_shutdown_is_executed_for_all_connections
    ...
    assert_equal [['shutdown']] * 3, recorders.map(&:calls)
  end
  ...
  def test_runs_shutdown_block_asynchronously_if_connection_was_in_use
    ...
    assert_equal [['shutdown'], ['shutdown'], []], recorders.map(&:calls)

    pool.checkin

    assert_equal [['shutdown'], ['shutdown'], ['shutdown']], recorders.map(&:calls)
  end
